...
void genkendDTMFtoner(double stor, double lille, paTestData *data){ 
    ...
    if(freq[0]-at < stor && freq[0]+at > stor){
        if (freq[4]-at < lille && freq[4]+at > lille){std::cout << "1" << std::endl; data->freqVals.push_back(49);} //49 = 1 (ASCII)
        if (freq[5]-at < lille && freq[5]+at > lille){std::cout << "4" << std::endl; data->freqVals.push_back(52);}
        if (freq[6]-at < lille && freq[6]+at > lille){std::cout << "7" << std::endl; data->freqVals.push_back(55);}
        if (freq[7]-at < lille && freq[7]+at > lille){std::cout << "*" << std::endl; data->freqVals.push_back(42);}
    }

    if(freq[1]-at < stor && freq[1]+at > stor){
        if (freq[4]-at < lille && freq[4]+at > lille){std::cout << "2" << std::endl; data->freqVals.push_back(50);}
        if (freq[5]-at < lille && freq[5]+at > lille){std::cout << "5" << std::endl; data->freqVals.push_back(53);}
        if (freq[6]-at < lille && freq[6]+at > lille){std::cout << "8" << std::endl; data->freqVals.push_back(56);}
        if (freq[7]-at < lille && freq[7]+at > lille){std::cout << "0" << std::endl; data->freqVals.push_back(48);}
    }

    if(freq[2]-at < stor && freq[2]+at > stor){
        if (freq[4]-at < lille && freq[4]+at > lille){std::cout << "3" << std::endl; data->freqVals.push_back(51);}
        if (freq[5]-at < lille && freq[5]+at > lille){std::cout << "6" << std::endl; data->freqVals.push_back(54);}
        if (freq[6]-at < lille && freq[6]+at > lille){std::cout << "9" << std::endl; data->freqVals.push_back(57);}
        if (freq[7]-at < lille && freq[7]+at > lille){std::cout << "#" << std::endl; data->freqVals.push_back(35);}
    }

    if(freq[3]-at < stor && freq[3]+at > stor){
        if (freq[4]-at < lille && freq[4]+at > lille){std::cout << "A" << std::endl; data->freqVals.push_back(65);}
        if (freq[5]-at < lille && freq[5]+at > lille){std::cout << "B" << std::endl; data->freqVals.push_back(66);}
        if (freq[6]-at < lille && freq[6]+at > lille){std::cout << "C" << std::endl; data->freqVals.push_back(67);}
        if (freq[7]-at < lille && freq[7]+at > lille){
            std::cout << "D" << std::endl; 
            data->freqVals.push_back(68);
